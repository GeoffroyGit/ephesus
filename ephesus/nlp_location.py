...
class TrainerLocation():
    def __init__(self, X, y,
        ...
        self.X = X
        self.y = y
        self.list_of_location_codes = list_of_location_codes
        self.tokenizer = None
        self.model = None
        self.X_train = None
        self.X_test = None
    ...
    def apply_tokenizer(self, X):
        ...
        # pad the sequences
        X_pad = pad_sequences(X_token, dtype='float32', padding='post', value=0, maxlen=25)
        return X_pad

    def train_location(self):
        '''
        This is a draft of a function to train a model to recognise the LOCATION code
        '''
        # preprocessing
        ...
        # on ne fait pas la suppression des stop_words
        # X["X"] = X["X"].apply(remove_stop_words)
        X = X["X"]

        # preprocess y
        y = self.y.copy()
        for code in self.list_of_location_codes:
        ...
        # nb_of_categories = len(self.list_of_location_codes) + 1 # +1 for the _unknown_ category
        ...
        self.model.fit(X_train_pad, self.y_train, batch_size=16, epochs=50, validation_split=0.3, callbacks=[es])
    ...
    def predict_location(self):
        ...
        df_pred_clean["location"] = df_pred["level_1"]
